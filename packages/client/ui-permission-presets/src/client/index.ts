...
function optionsOf(value: PermissionSelect, t: (key: string) => string): SelectOption[] {
  const labels = {
    readOnly: t('preset.readOnly'),
    workspaceWrite: t('preset.workspaceWrite'),
    fullAccess: t('preset.fullAccess'),
  }
  return value.options
    .filter(option => option.value !== 'custom')
    .map(option => ({
      id: option.value,
      label: displayPermissionPreset(option.value, option.name, labels),
