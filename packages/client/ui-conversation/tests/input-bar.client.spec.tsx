...
describe('command launcher chrome and control seats', () => {
  ...
  it('switches built-in permission labels through the English locale mapping', () => {
    const permissions = {
      options: [
        { value: 'read-only', name: 'read-only' },
        { value: 'workspace-write', name: 'workspace-write' },
    ...
    const { view } = bench({ permissions, t: makeTranslate(en, commonEn) })
    const trigger = view.getByLabelText('Access mode, current: Workspace Write')
